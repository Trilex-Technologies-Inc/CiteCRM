			...
			<table class="table table-hover table-striped mb-0">
				...
				{foreach from=$new item=new}
					{if $new.WORK_ORDER_ID > 0}
						<tr style="cursor:pointer;" ondblclick="window.location='?page=workorder:view&wo_id={$new.WORK_ORDER_ID}&customer_id={$new.CUSTOMER_ID}&page_title={$translate_workorder_page_title} {$new.WORK_ORDER_ID}';">
							<td>
							...
							<td>{$new.WORK_ORDER_SCOPE}</td>
							<td>{$new.CONFIG_WORK_ORDER_STATUS}</td>
							<td nowrap>
					...
					{else}
						<tr>
							<td colspan="7" class="text-center text-danger">{$translate_workorder_msg_1}</td>
						</tr>
					{/if}
				{/foreach}
				</tbody>
