										...
										<tr>
											<td>
												{section name=b loop=$work_order_notes}
													<p><b>{$translate_workorder_enter_by} </b>{$work_order_notes[b].EMPLOYEE_DISPLAY_NAME}  <b>{$translate_workorder_date} </b> {$work_order_notes[b].WORK_ORDER_NOTES_DATE|date_format:"%m-%d-%Y"}<br>
