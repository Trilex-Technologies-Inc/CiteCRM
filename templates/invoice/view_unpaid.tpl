										...
										<table class="menutable" width="100%" border="0" cellpadding="5" cellspacing="0">
											<tr>
												...
												<form id="1">
													...
													{if $previous != ''}
														<a href="?page=invoice:view_unpaid&name={$name}&submit=submit&page_no={$previous}"><img src="images/back_24.gif" border="0"></a>&nbsp;
													{/if}
													...
													{if $next != ''}
													<a href="?page=invoice:view_unpaid&name={$name}&submit=submit&page_no={$next}"><img src="images/forwd_24.gif" border="0"></a>
													{/if}
													
													<a href="?page=invoice:view_unpaid&name={$name}&submit=submit&page_no={$total_pages}"><img src="images/fastf_24.gif" border="0"></a>
													<br>
												...
												<td valign="top" colspan="2">
														<table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
															...
															<tr onmouseover="this.className='row2'" onmouseout="this.className='row1'" onDblClick="window.location='index.php?page=invoice:new&wo_id={$invoice[q].WORKORDER_ID}&page_title={$translate_invoice_invoice}&customer_id={$invoice[q].CUSTOMER_ID}';" class="row1">
																...
																<td class="olotd4" nowrap><a href="index.php?page=workorder:view&wo_id={$invoice[q].WORKORDER_ID}&page_title=Work%20Order%20ID%20{$invoice[q].WORKORDER_ID}">{$invoice[q].WORKORDER_ID}</a></td>
																<td class="olotd4" nowrap><img src="images/icons/16x16/view+.gif" border="0" onMouseOver="ddrivetip('<center><b>Contact</b></center><hr><b>Work: </b>{$invoice[q].EMPLOYEE_WORK_PHONE}<br><b>Mobile: </b>{$invoice[q].EMPLOYEE_MOBILE_PHONE}<br><b>Home: </b>{$invoice[q].EMPLOYEE_HOME_PHONE}')" onMouseOut="hideddrivetip()"><a  href="?page=employees:employee_details&employee_id={$invoice[q].EMPLOYEE_ID}&page_title={$invoice[q].EMPLOYEE_DISPLAY_NAME}">{$invoice[q].EMPLOYEE_DISPLAY_NAME}</td>
																<td class="olotd4" nowrap>${$invoice[q].SUB_TOTAL}</td>
																<td class="olotd4" nowrap>${$invoice[q].TAX}</td>
																<td class="olotd4" nowrap>${$invoice[q].DISCOUNT}</td>
																<td class="olotd4" nowrap>${$invoice[q].INVOICE_AMOUNT}</td>
