...
/* Assigned counts */
$q = 'SELECT count(*) as count FROM '.PRFX.'TABLE_WORK_ORDER WHERE  WORK_ORDER_CURENT_STATUS='.$db->qstr(2);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
...
$q = 'SELECT count(*) as count FROM '.PRFX.'TABLE_WORK_ORDER WHERE  WORK_ORDER_CURENT_STATUS='.$db->qstr(3);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
...
/* waiting for payment */
$q = 'SELECT count(*) as count FROM '.PRFX.'TABLE_WORK_ORDER WHERE  WORK_ORDER_CURENT_STATUS='.$db->qstr(7);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
...
/* invoice stats */
/* unpaid Invoices */
$q = 'SELECT count(*) as count FROM '.PRFX.'TABLE_INVOICE WHERE INVOICE_PAID='.$db->qstr(0).' AND  BALLANCE='.$db->qstr(0);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
...
/* sum of unpaid */
$q = 'SELECT SUM(INVOICE_AMOUNT) as sum FROM '.PRFX.'TABLE_INVOICE WHERE INVOICE_PAID='.$db->qstr(0).' AND  BALLANCE='.$db->qstr(0);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
...
/* sum */
$q = 'SELECT SUM(BALLANCE) as sum FROM '.PRFX.'TABLE_INVOICE WHERE INVOICE_PAID='.$db->qstr(0).' AND  BALLANCE >'.$db->qstr(0);
if(!$rs = $db->execute($q)){
	force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1');
	exit;
} else {
