...
if($invocie_details['INVOICE_AMOUNT'] > $gift_amount){

	if($invocie_details['BALLANCE'] > 0 ) {
		$balance = $invocie_details['BALLANCE'] - $gift_amount;
	} else {
		$balance = $invocie_details['INVOICE_AMOUNT'] - $gift_amount; 
	}	
	$paid_amount = $gift_amount + $invocie_details['PAID_AMOUNT'];
	...
	if($invocie_details['INVOICE_AMOUNT'] < $gift_amount) {
			...
			$remain_gift = $gift_amount - $invocie_details['INVOICE_AMOUNT'];
			...
			$gift_amount = $invocie_details['INVOICE_AMOUNT'];
			$flag = 1
	} 
