...
$an_password 	= decrypt ($rs->fields['AN_PASSWORD'],$strKey);
...
$inv_msg = $rs->fields['COMPANY_NAME']."(Phone ".$rs->fields['COMPNAY_PHONE'].") Repair Charge";
...
if($result[0] == "1") {
	...
	if($invocie_details['INVOICE_AMOUNT'] > $cc_amount){
		if($invocie_details['BALLANCE'] > 0 ) {
			$balance = $invocie_details['BALLANCE'] - $cc_amount;
		} else {
			$balance = $invocie_details['INVOICE_AMOUNT'] - $cc_amount; 
		}	
		$paid_amount = $cc_amount + $invocie_details['PAID_AMOUNT'];
