...
$q = "SELECT WORK_ORDER_STATUS,WORK_ORDER_CURENT_STATUS FROM ".PRFX."TABLE_WORK_ORDER WHERE WORK_ORDER_ID=".$db->qstr($wo_id);
if(!$rs = $db->execute($q)) {
...
} elseif ($rs->fields['WORK_ORDER_CURENT_STATUS'] == 3) {
...
if(isset($VAR["submit"])){
	...
	if (!close_work_order($db,$VAR)){
		force_page('workorder', "view&wo_id=$wo_id&error_msg=Faild to Close Work Order.&page_title=Work Order ID $wo_id");
	} else {
