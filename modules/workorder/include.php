...
# Work Order Include File                       #
# Version 0.0.2 Fri Nov 18 23:36:57 PST 2005    #
#                                               #
#################################################
...
function display_workorders($db, $page_no, $where){
    ...
    if (preg_match('/WORK_ORDER_STATUS\s*=\s*([0-9]+)/i', $where, $matches)) {
        $status = $matches[1];
    }
    ...
    $sql = "SELECT 
            ...
            ".PRFX."TABLE_WORK_ORDER.WORK_ORDER_CURENT_STATUS,
            ".PRFX."TABLE_CUSTOMER.CUSTOMER_ID,
            ".PRFX."TABLE_CUSTOMER.CUSTOMER_DISPLAY_NAME,
            ".PRFX."TABLE_CUSTOMER.CUSTOMER_ADDRESS,
            ".PRFX."TABLE_CUSTOMER.CUSTOMER_CITY,
...
#########################################
# Display Customer Contact Informatuion #
#########################################
...
function insert_new_workorder($db,$VAR){
    ...
    $sql = "INSERT INTO ".PRFX."TABLE_WORK_ORDER  SET 
            ...
            WORK_ORDER_CURENT_STATUS    =".$db->qstr(1).",
...
function update_status($db,$VAR){
    ...
    $sql = "UPDATE ".PRFX."TABLE_WORK_ORDER SET
              WORK_ORDER_CURENT_STATUS    =". $db->qstr( $VAR["status"] ).",
              LAST_ACTIVE                    =". $db->qstr(time())."
...
function insert_new_note($db,$VAR){
    ...
    $sql = "INSERT INTO ".PRFX."TABLE_WORK_ORDER_NOTES SET 
             WORK_ORDER_ID                        =". $db->qstr( $VAR["wo_id"]                    ).", 
             WORK_ORDER_NOTES_DESCRIPTION    =". $db->qstr( $VAR["work_order_notes"]        ).", 
...
###############################
#    Get Work Order Schedual        #
###############################
function get_work_order_schedual ($db,$wo_id){
    $sql = "SELECT * FROM ".PRFX."TABLE_SCHEDUAL WHERE WORK_ORDER_ID=".$db->qstr($wo_id); 
    if(!$rs = $db->Execute($sql)) {
        force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1&type=database');
        exit;
    }
    
    $schedual = $rs->GetArray();
    return $schedual;
}
