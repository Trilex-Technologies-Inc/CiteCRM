...
if($rs->fields['WORK_ORDER_STATUS'] != '6') {
		...
		$q = "SELECT count(*) as count  FROM ".PRFX."TABLE_SCHEDUAL WHERE WORK_ORDER_ID=".$db->qstr($wo_id);
			if(!$rs = $db->execute($q)) {
				force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1&type=database');
				exit;
			}
		...
		$q = "UPDATE ".PRFX."TABLE_WORK_ORDER SET WORK_ORDER_CURENT_STATUS =".$db->qstr($status).", LAST_ACTIVE=".$db->qstr(time())." WHERE WORK_ORDER_ID = ".$db->qstr($wo_id) ;
			if(!$rs = $db->execute($q)) {
				force_page('core', 'error&error_msg=MySQL Error: '.$db->ErrorMsg().'&menu=1&type=database');
				exit;
		}
